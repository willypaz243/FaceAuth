...
class K_mean():
    ...
    def train(self, dataset, graficar=False):
        ...
        while diferencia.max() > 0:
            ...
            for i in range(self.grupos):
                distancias.append(
                    ((dataset-self.centroides[i])**2).sum(axis=1)**0.5)
    ...
    def that_class(self, entrada):
        ...
        if not len(self.centroides) == 0:
            ...
            # debe de estar lo suficientemente cerca.
            if minimo < self.radios[indice]:
                return int(self.ids[indice])
            else:
                return -1
        else:
            return -1
        ...
        if codes.any():
            ...
            if list(self.ids).count(id_user) >= 1:
                ...
                if not self.centroides.any():
                    ...
                    new_radio = np.linalg.norm(
                        codes - new_centroid, axis=1).max()
                    self.radios.append(new_radio)
                else:
                    self.centroides = np.append(
                        self.centroides, [new_centroid], axis=0)
    ...
    def save_model(self):
        if not os.path.exists("dataset"):
            os.mkdir("dataset")
        np.savetxt(f"dataset/{self.model_name}_dataset.csv",
                   self.dataset, fmt='%r', delimiter=',')
        database = np.concatenate(
            [np.transpose([self.ids]), self.centroides], axis=1)
        np.savetxt(f'dataset/{self.model_name}_database.csv',
                   database, fmt='%r', delimiter=',')

    def load_model(self):

        if os.path.exists(f"dataset/{self.model_name}_dataset.csv"):
            self.dataset = np.loadtxt(
                f"dataset/{self.model_name}_dataset.csv", delimiter=',')
            if len(self.dataset.shape) < 2:
                self.dataset = np.array([self.dataset])
        if os.path.exists(f'dataset/{self.model_name}_database.csv'):
            database = np.loadtxt(
                f'dataset/{self.model_name}_database.csv', delimiter=',')
    ...
    def __call__(self, entrada):
        return self.that_class(entrada)
