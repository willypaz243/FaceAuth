...
from face_auth import FaceDetector, FaceImgProcessor, FaceEncoder, K_mean, Identifier

FREEZE_MODEL_PATH = './frozen_models/frozen_graph.pb'

face_detector = FaceDetector()

landmark_file = "face_auth/datas/shape_predictor_68_face_landmarks.dat"
img_processor = FaceImgProcessor(landmark_file)

net_model = cv2.dnn.readNet(FREEZE_MODEL_PATH)
print(type(net_model))
...
# dependiendo al numero de camaras que esten conectadas, 0, 1, 2 ...
device_cam = 1
...
def identify():
    ...
    while activo:
        ...
        cv2.imshow('registor', frame)
        activo = not cv2.waitKey(1) == ord('q')
    cv2.destroyAllWindows()
    cap.release()
